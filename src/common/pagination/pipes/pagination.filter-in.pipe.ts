...
export function PaginationFilterInPipe<T>(
    ...
    class MixinPaginationFilterInPipe implements PipeTransform {
        ...
        async transform(value: string): Promise<any> {
            ...
            let finalValue: T[] = this.helperArrayService.unique<T>(
                value?.split(',') as T[]
            );

            // With TypeORM, string IDs are used directly - no ObjectId conversion needed

            return finalValue?.length ? this.databaseService.filterIn<T>(field, finalValue) : undefined;
        }
