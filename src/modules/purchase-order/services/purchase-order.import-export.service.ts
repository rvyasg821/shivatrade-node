...
export class PurchaseOrderImportExportService {
     ...
     * `PurchaseOrderService.preClose()` the single-document button uses — no
    ...
    async importPreClose(
        ...
        const rows =
            pickSheet(
                sheets as any,
                ['PreClose', 'Pre-Close', 'SalesOrders'],
                0
            ) || [];
        if (!rows.length) {
            throw new BadRequestException(
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const existing: any = await this.poRepository.findOne({
                company_id: companyId,
            ...
            if (
                existing.status !== ENUM_PURCHASE_ORDER_STATUS.CONFIRMED &&
                existing.status !== ENUM_PURCHASE_ORDER_STATUS.IN_PROCESS
            ) {
                skipped.push({
                    row: rowNum,
                    voucher_no: voucherNo,
                    reason: `Only a confirmed/in-process SO can be pre-closed (current: ${existing.status})`,
                });
                continue;
            }
            const date =
